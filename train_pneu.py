...
#           .format(top1=top1, losses=losses))
    print('Acc1GX: ', top1.avg)
    print('LossGX: ', losses.avg)
...
def main():
    ...
    # NEED TO CHANGE
    parser.add_argument('--transfer-resume', type=str, default='../paths/luna/ssl/1/best.pth.tar',# ./checkpoint/inat_resnet50_tl/best.pth.tar
                        help='Path to load transfering pretrained model')
    parser.add_argument('--checkpoint-path',type = str, default='../checkpoints_target/luna/ssl/1/',
    ...
    parser.add_argument('--root-dir', type=str, default='../../../../../../datasets/target/xray/chest_xray/chest_xray/',
...
#     testset = Flower102(root_dir=args.root_dir, mode = 'test', transforms=test_trans)
    ...
    valset = ImageFolder(
        root=os.path.join(args.root_dir, 'val'),
...
#                 assert set(msg.missing_keys) == {"fc.weight", "fc.bias"}
    ...
    sheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10) # used to be 10
